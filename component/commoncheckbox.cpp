...
CommonCheckBox::CommonCheckBox(QWidget *parent, const QString &picture)
	: QWidget(parent)
{
//    qDebug() << "init main.....";
...
//    name_label->setText(tr("Records Count:"));
...
int CommonCheckBox::getCheckBoxStatus()
{
...
void CommonCheckBox::onZoneClicked()
{
    if(checkbox->checkState() == 0) {
        checkbox->setCheckState(Qt::Checked);
...
void CommonCheckBox::resetMainStatus(int status)
{
    if(status == 0) {
        checkbox->setCheckState(Qt::Unchecked);
    }
...
void CommonCheckBox::initConnect()
{
...
void CommonCheckBox::setLabelText(const QString &title, int count)
{
