...
void BatteryInfo::onUpdateInfo(QString strInfoJson)
{
    ...
    qInfo()<<"battery Info:"<<m_strInfoJson;
    QTimer::singleShot(0, this, [&, this, strInfoJson](){
        this->updateInfoItems(strInfoJson);
    });
}

void BatteryInfo::initConnections()
{
    connect(DataWorker::getInstance(), SIGNAL(updateBatteryInfo(uint,QString)), this, SLOT(onUpdateInfo(uint,QString)));
}

void BatteryInfo::clearInfoItems()
{
...
void BatteryInfo::addInfoItems(unsigned uGroup, unsigned uId, QString strTitle, QString strContent, bool bMulti)
{
         ...
         if (valJson.isArray()) {
            ...
            for (int n = 0; n < arrayJson.size(); n++) {
                if (arrayJson.at(n).isObject()) {
                    ...
                    QJsonValue subItemValue = subItemObj.value(BTI_SERAILNUM);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_SERAILNUM, tr("Serail Number"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_MANUFACTURER);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_MANUFACTURER, tr("Manufacturer"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_MODEL);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_MODEL, tr("Model"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_STATE);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_STATE, tr("State"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_PERCENTAGE);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_PERCENTAGE, tr("Percentage"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_ENERGY);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_ENERGY, tr("Energy"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_ENERGYFULL);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_ENERGYFULL, tr("Energy Full"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_TTE);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_TTE, tr("Time To Empty"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(BTI_USEDTIMES);
                    if (subItemValue.isString()) {
                        addInfoItems(n, BATTERYINFO_USEDTIMES, tr("Used Times"), subItemValue.toString(), bMulti);
                    }
                }
            }
         }
...
void BatteryInfo::onRefreshInfo()
{
    DataWorker::getInstance()->getBatteryInfo();
}
