...
KeyboardInfo::KeyboardInfo(QWidget *parent)
...
void KeyboardInfo::onUpdateInfo(QString strInfoJson)
{
    ...
    qInfo()<<"KeyboardInfo Info:"<<m_strInfoJson;
...
void KeyboardInfo::initConnections()
{
    connect(DataWorker::getInstance(), SIGNAL(updateKeyboardInfo(uint,QString)), this, SLOT(onUpdateInfo(uint,QString)));
}

void KeyboardInfo::clearInfoItems()
{
...
void KeyboardInfo::addInfoItems(unsigned uGroup, unsigned uId, QString strTitle, QString strContent, bool bMulti)
{
         ...
         if (valJson.isArray()) {
            ...
            for (int n = 0; n < arrayJson.size(); n++) {
                if (arrayJson.at(n).isObject()) {
                    ...
                    QJsonValue subItemValue = subItemObj.value(KBI_NAME);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_NAME, tr("Name"), subItemValue.toString(), bMulti);
                    }
//                    subItemValue = subItemObj.value(KBI_DEVTYPE);
//                    if (subItemValue.isString()) {
//                        addInfoItems(n, KEYBOARDINFO_DEVTYPE, tr("Type"), subItemValue.toString(), bMulti);
//                    }
                    subItemValue = subItemObj.value(KBI_DEVMODEL);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_DEVMODEL, tr("Model"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(KBI_MANUFACTURER);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_MANUFACTURER, tr("Manufacurer"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(KBI_INTERFACE);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_INTERFACE, tr("Interface"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(KBI_DRIVER);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_DRIVER, tr("Driver"), subItemValue.toString(), bMulti);
                    }
                    subItemValue = subItemObj.value(KBI_DEVADDR);
                    if (subItemValue.isString()) {
                        addInfoItems(n, KEYBOARDINFO_DEVADDR, tr("Device Address"), subItemValue.toString(), bMulti);
                    }
                }
            }
         }
...
void KeyboardInfo::onRefreshInfo()
{
    DataWorker::getInstance()->getKeyboardInfo();
}

void KeyboardInfo::chkNoDevice()
{
    unsigned infoSize = 0;
    QMap<unsigned,QMap<unsigned, QWidget*>>::iterator itMapInfoItems = m_mapInfoItems.begin();
    for (;itMapInfoItems != m_mapInfoItems.end(); itMapInfoItems++) {
    ...
    if (infoSize == 0) {
        addInfoItems(0, KEYBOARDINFO_INVALID, "", tr("Device not exitst or Get Device is Empty"), false);
    }
}
